...
Safety Notes:
  ...
  - ObjectID filtering helps target specific models/environments for analysis/deletion
    ...
    parser.add_argument(
        "--apply", action="store_true", help="For delete_image: Actually delete images (default is dry-run)"
    )

    parser.add_argument(
        "--force", action="store_true", help="For delete_image: Skip confirmation prompt when using --apply"
    )

    parser.add_argument("--file", help="File containing ObjectIDs (one per line) to filter images (for delete_image)")

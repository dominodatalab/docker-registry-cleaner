    ...
    try:
        ...
        if object_ids:
            ...
            for user_doc in users:
                ...
                login_id = user_doc.get('loginId', {}).get('id', '') if isinstance(user_doc.get('loginId'), dict) else ''
                if login_id:
...
def build_environment_tag_to_metadata_mapping(
    environment_tags: Set[str],
    include_env_name: bool = True,
    include_owner_info: bool = True
) -> Dict[str, Dict]:
    ...
    try:
        ...
        if include_env_name and environment_ids:
            environments = db.environments_v2.find(
                {'_id': {'$in': list(environment_ids)}},
                {'_id': 1, 'name': 1}
            )

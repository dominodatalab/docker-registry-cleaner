...
class HealthChecker:
        ...
        try:
            ...
            try:
                actionable_error = create_mongodb_connection_error(
                    config_manager.get_mongo_host(),
                    config_manager.get_mongo_port(),
                    e
                )
                error_message = actionable_error.message
            except:
    ...
    def check_s3_access(self) -> HealthCheckResult:
        ...
        try:
            ...
            try:
                actionable_error = create_s3_error("head_bucket", s3_bucket, Exception("AWS credentials not configured"))
                error_message = actionable_error.message
            except:

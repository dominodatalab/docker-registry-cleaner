#!/usr/bin/env python3
"""
Find and optionally delete archived environment tags in MongoDB and Docker registry.

This script queries MongoDB for archived environment ObjectIDs and finds matching
Docker tags in the registry. Can optionally delete the Docker images and clean up
MongoDB records.

Workflow:
- Query MongoDB environments_v2 collection for documents where isArchived == true
- Query environment_revisions collection for documents with environmentId matching archived environments
- Extract ObjectIDs from both archived environments and their revisions
...
Usage examples:
  ...
  python delete_archived_env_tags.py --registry-url docker-registry:5000 --repository dominodatalab
  
  # Delete archived environment tags directly
  python delete_archived_env_tags.py --apply
  
  # Delete archived environment tags from pre-generated file
  python delete_archived_env_tags.py --apply --input archived-tags.json
"""
...
class ArchivedTagsFinder:
    """Main class for finding and managing archived environment tags"""
    ...
    def fetch_archived_object_ids(self) -> List[str]:
        """Fetch archived ObjectIDs from MongoDB environments_v2 collection"""
        ...
        try:
            ...
            # Query for archived environments
            ...
            for doc in cursor:
                _id = doc.get("_id")
                if _id is not None:
                    archived_environment_ids.append(str(_id))
            ...
            if archived_environment_ids:
                ...
                # Find environment revisions that belong to archived environments
                ...
                for doc in revision_cursor:
                    _id = doc.get("_id")
                    if _id is not None:
                        archived_revision_ids.append(str(_id))
                
                self.logger.info(f"Found {len(archived_revision_ids)} environment revision ObjectIDs for archived environments")
            
            # Combine both sets of IDs (environment IDs and revision IDs)
            all_archived_ids = list(set(archived_environment_ids + archived_revision_ids))
    ...
    def find_matching_tags(self, archived_ids: List[str]) -> List[ArchivedTagInfo]:
    ...
    def delete_archived_tags(self, archived_tags: List[ArchivedTagInfo]) -> Dict[str, int]:
        ...
        try:
            ...
            if object_ids_to_clean:
                self.logger.info(f"Cleaning up {len(object_ids_to_clean)} environment_revisions records from MongoDB...")
                mongo_client = get_mongo_client()
    ...
    def generate_report(self, archived_ids: List[str], archived_tags: List[ArchivedTagInfo], freed_space_bytes: int = 0) -> Dict:
        ...
        # Separate ObjectIDs by type (we need to re-fetch to distinguish between environment and revision IDs)
        mongo_client = get_mongo_client()
        environment_ids = []
        revision_ids = []
        ...
        try:
            ...
            # Check which IDs are environment IDs
            from bson import ObjectId
            for obj_id_str in archived_ids:
                try:
                    obj_id = ObjectId(obj_id_str)
                    if environments_collection.find_one({"_id": obj_id}):
                        environment_ids.append(obj_id_str)
                    else:
                        revision_ids.append(obj_id_str)
                except:
                    # If it's not a valid ObjectId, assume it's a revision ID
                    revision_ids.append(obj_id_str)
        finally:
            mongo_client.close()
        ...
        summary = {
            ...
            'archived_environment_ids': len(environment_ids),
            'archived_revision_ids': len(revision_ids),
        ...
        report = {
            ...
            'archived_environment_ids': environment_ids,
            'archived_revision_ids': revision_ids,
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Find and optionally delete archived environment tags in Docker registry",
...
Examples:
  ...
  python delete_archived_env_tags.py --registry-url registry.example.com --repository my-repo

  # Custom output file
  python delete_archived_env_tags.py --output archived-tags.json
  ...
  python delete_archived_env_tags.py --apply --force
        """
    )
    ...
    parser.add_argument(
        '--apply',
        action='store_true',
        help='Actually delete archived environment tags and clean up MongoDB (default: dry-run)'
    )
...
def main():
    ...
    try:
        ...
        logger.info("=" * 60)
        if is_delete_mode:
            logger.info("   Deleting archived environment tags")
        else:
            logger.info("   Finding archived environment tags")
        logger.info("=" * 60)
        ...
        # Create finder
        finder = ArchivedTagsFinder(registry_url, repository)
        ...
        if use_input_file:
            ...
            if not archived_ids:
                logger.info("No archived environment ObjectIDs found")
        ...
        if is_delete_mode:
            ...
            if not args.force:
                logger.warning(f"\n⚠️  WARNING: About to delete {len(archived_tags)} archived environment tags!")
            ...
            logger.info(f"\n🗑️  Deleting {len(archived_tags)} archived environment tags...")
            ...
            logger.info("\n✅ Archived environment tags deletion completed successfully!")
            ...
            logger.info("\n" + "=" * 60)
            logger.info("   ARCHIVED ENVIRONMENT TAGS ANALYSIS SUMMARY")
            logger.info("=" * 60)
            ...
            if archived_tags:
                logger.warning(f"\n⚠️  Found {len(archived_tags)} archived environment tags that may need cleanup!")
                logger.info("Review the detailed report to identify which Docker images are associated with archived environments.")
                ...
                logger.info("\n✅ No archived environment tags found!")
            
            logger.info("\n✅ Archived environment tags analysis completed successfully!")

...
def build_tag_to_owners_mapping(
    analyzer: ImageAnalyzer, mongodb_reports: Dict[str, List[Dict]]
    ...
    for image_id, image_data in analyzer.images.items():
        ...
        has_usage = bool(
            ...
            and any(
                usage_info.get("runs")
                or usage_info.get("workspaces")
                or usage_info.get("models")
                or usage_info.get("projects")
                or usage_info.get("scheduler_jobs")
                or usage_info.get("organizations")
                or usage_info.get("app_versions")
            )
        )

        # Extract owners from usage info (pass both mappings)
        owners = extract_owners_from_usage_info(
            tag, usage_info, mongodb_reports, tag_to_model_version_created_by, tag_to_author_map
        )
...
def generate_user_size_report(
    analyzer: ImageAnalyzer, mongodb_reports: Dict[str, List[Dict]], image_types: List[str] = None
) -> Dict:
...
def print_report_summary(report_data: Dict) -> None:
    ...
    logger.info(
        f"Total Space Freed (if all deleted): {sizeof_fmt(summary['total_freed_if_all_deleted_bytes'])} ({summary['total_freed_if_all_deleted_gb']} GB)"
    )
    logger.info("=" * 80)
    ...
    logger.info("-" * 165)
    logger.info(
        f"{'Rank':<6} {'User ID':<30} {'User Name':<30} {'Login ID':<20} {'Images':<10} {'Total Size':<15} {'Freed if Deleted':<20} {'Environments':<15} {'Models':<15}"
    )
    logger.info("-" * 165)
    ...
    for idx, user in enumerate(users[:20], 1):
        ...
        login_id_display = (
            user.get("login_id", "")[:17] + "..." if len(user.get("login_id", "")) > 20 else user.get("login_id", "")
        )
        logger.info(
...
Examples:
  ...
  python user_size_report.py --output custom-report.json
    ...
    parser.add_argument(
        "--output", help="Output file path for the report (default: user-size-report.json in reports directory)"
    )
    ...
    parser.add_argument(
        "--max-workers", type=int, help="Maximum number of parallel workers for image analysis (default: from config)"
    )

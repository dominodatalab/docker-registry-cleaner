...
class DeactivatedUserEnvFinder(BaseDeletionScript):
    ...
    def _check_cloned_revision_chain(
        self, cloned_rev_id: str, deletion_set: set, environment_revisions_collection, visited: set = None
    ) -> bool:
    ...
    def _filter_cloned_dependencies(
        self, environment_ids: List[str], revision_ids: List[str]
        ...
        try:
            ...
            cursor = revs.find(
                {"_id": {"$in": [ObjectId(x) for x in revision_ids if len(x) == 24]}},
    ...
    def fetch_deactivated_user_env_ids(self) -> Tuple[List[str], List[str], Dict[str, Dict]]:
        ...
        for kc_user in kc_users:
            if not kc_user.get("enabled", True):  # User is deactivated
                try:
                    ...
                    deactivated_user_ids[domino_user_id] = {"email": email, "keycloak_id": kc_user["id"]}
        ...
        try:
            ...
            query = {"ownerId": {"$in": deactivated_owner_ids}, "visibility": "Private"}
            ...
            if environment_ids:
                ...
                revision_cursor = environment_revisions_collection.find(
                    {"environmentId": {"$in": environment_object_ids}}, {"_id": 1, "environmentId": 1}
                )
    ...
    def calculate_freed_space(self, deactivated_user_tags: List[DeactivatedUserEnvInfo]) -> int:
        ...
        try:
            ...
            self.logger.info(
                "This analyzes all images (not just deactivated user images) to count shared layer references correctly."
            )
    ...
    def delete_deactivated_user_envs(
        self,
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    f"Deleting {len(unique_tags)} unique Docker images from registry ({len(deactivated_user_tags)} total references)..."
                )
                if len(unique_tags) < len(deactivated_user_tags):
                    self.logger.info(
                        f"  Note: {len(deactivated_user_tags) - len(unique_tags)} tags contain multiple deactivated user ObjectIDs"
                    )
                ...
                if resume:
                    remaining_tags = self.checkpoint_manager.get_remaining_items(
                        "delete_unused_private_environments", tag_identifiers, operation_id
                    )
                    if remaining_tags:
                        self.logger.info(
                    ...
                    self.checkpoint_manager.save_checkpoint(
                        "delete_unused_private_environments", [], len(tag_identifiers), operation_id=operation_id
                    )
                ...
                for idx, ((image_type, tag), data) in enumerate(unique_tags.items()):
                    ...
                    if tag_info.tag in in_use_tags:
                        ...
                        usage_summary = service.generate_usage_summary(usage)
                        self.logger.warning(
                            f"  Skipping {tag_info.full_image} (user: {tag_info.user_email}, in use: {usage_summary})"
                        )
                ...
                if mongo_cleanup:
                    ...
                    if skipped_env_ids > 0 or skipped_rev_ids > 0:
                        self.logger.info(
                            f"Skipping MongoDB cleanup for {skipped_env_ids + skipped_rev_ids} ObjectIDs due to Docker deletion failures"
                        )
                    ...
                    try:
                        ...
                        if rev_ids_to_clean:
                            self.logger.info(
                                f"Cleaning up {len(rev_ids_to_clean)} environment_revisions records from MongoDB..."
                            )
                        ...
                        if env_ids_to_clean:
                            self.logger.info(
                                f"Cleaning up {len(env_ids_to_clean)} environments_v2 records from MongoDB..."
                            )
                            environments_collection = db["environments_v2"]
                            revisions_collection = db["environment_revisions"]
                            ...
                            for obj_id_str in env_ids_to_clean:
                                try:
                                    ...
                                    remaining_revs = revisions_collection.count_documents(
                ...
                if total_processed > 0:
                    ...
                    checkpoint = self.checkpoint_manager.load_checkpoint(
                        "delete_unused_private_environments", operation_id
                    )
                    if checkpoint:
                        total_completed = (
    ...
    def generate_report(
        ...
        for tag in deactivated_user_tags:
            ...
            if email not in by_user:
                by_user[email] = {"user_id": tag.user_id, "tags": [], "environments": set(), "tag_count": 0}
...
Environment Variables Required:
  ...
  KEYCLOAK_PASSWORD
    ...
    parser.add_argument("--output", help="Output file path (default: reports/deactivated-user-envs.json)")

    parser.add_argument(
        "--apply", action="store_true", help="Actually delete environments and images (default: dry-run)"
    )

    parser.add_argument("--input", help="Input file containing pre-generated report to delete")
    ...
    try:
        ...
        if is_delete_mode:
            ...
            if not finder.confirm_deletion(
                len(deactivated_user_tags), "tags for private environments owned by deactivated users", force=args.force
            ):
                logger.info("Operation cancelled by user")
                sys.exit(0)
            ...
            if args.apply and args.run_registry_gc:
                ...
                logger.info("Running Docker registry garbage collection after deactivated user environment deletion...")

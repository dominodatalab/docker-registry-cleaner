...
class UnusedEnvironmentsFinder(BaseDeletionScript):
    ...
    def _check_cloned_revision_chain(
        self, cloned_rev_id: str, deletion_set: set, environment_revisions_collection, visited: Optional[set] = None
    ) -> bool:
        ...
        if cloned_rev_id not in deletion_set:
            return False
        doc = environment_revisions_collection.find_one(
        ...
        if nested is not None:
            if not self._check_cloned_revision_chain(
                str(nested), deletion_set, environment_revisions_collection, visited
            ):
                return False
        return True
    ...
    def _filter_cloned_dependencies(self, unused_envs: List[UnusedEnvInfo]) -> List[UnusedEnvInfo]:
        ...
        try:
            ...
            cursor = revs.find(
                {"_id": {"$in": [ObjectId(x) for x in deletion_set if len(x) == 24]}},
                {"clonedEnvironmentRevisionId": 1, "environmentId": 1},
            )
            for doc in cursor:
            ...
            if skipped:
                self.logger.info(
                    f"Filtered out {skipped} environment/revision IDs due to cloned dependencies not in deletion set"
                )
            return filtered
        finally:
            mongo_client.close()
    ...
    def extract_used_environment_ids(
        self,
        model_env_data: List[dict],
    ...
    def fetch_all_environments_and_defaults(self) -> tuple:
        ...
        try:
            ...
            # Get organization default environments (collection may not exist)
            org_default_env_ids = set()
            if "organizations" in db.list_collection_names():
                organizations_collection = db["organizations"]
                org_cursor = organizations_collection.find(
                    {"defaultV2EnvironmentId": {"$ne": None}},
                    {"defaultV2EnvironmentId": 1},
                )
                for doc in org_cursor:
                    env_id = doc.get("defaultV2EnvironmentId")
                    if env_id is not None:
                        org_default_env_ids.add(str(env_id))
                self.logger.info(f"Found {len(org_default_env_ids)} organization default environments")
            else:
                self.logger.info(
                    "Collection 'organizations' not found, skipping organization default environment check"
                )
            ...
            if "model_products" in collection_names and "app_versions" in collection_names:
                ...
                model_products_cursor = model_products_collection.find({"isArchived": False}, {"_id": 1})
                ...
                if unarchived_product_ids:
                    ...
                    app_versions_cursor = app_versions_collection.find(
                        {"appId": {"$in": list(unarchived_product_ids)}}, {"environmentId": 1}
                    )
                    ...
                    self.logger.info(
                        f"Found {len(app_version_env_ids)} app version environments from unarchived products"
                    )
                else:
    ...
    def find_unused_environments(self) -> List[str]:
        ...
        used_ids = self.extract_used_environment_ids(
            model_env_data, workspace_env_data, runs_env_data, recent_days=self.recent_days
        )
            ...
            try:
                ...
                if resume:
                    remaining_tags = self.checkpoint_manager.get_remaining_items(
                        "delete_unused_environments", tag_identifiers, operation_id
                    )
                    if remaining_tags:
                        self.logger.info(
                    ...
                    self.checkpoint_manager.save_checkpoint(
                        "delete_unused_environments", [], len(tag_identifiers), operation_id=operation_id
                    )

                self.logger.info(
                    f"Deleting {len(unique_tags)} unique Docker images from registry ({len(unused_tags)} total references)..."
                )
                if len(unique_tags) < len(unused_tags):
                    self.logger.info(
                        f"  Note: {len(unused_tags) - len(unique_tags)} tags contain multiple unused ObjectIDs"
                    )
                ...
                # This is a REAL-TIME check that happens right before deletion to catch:
                # - New pods that started after the initial analysis
                ...
                if in_use_tags:
                    self.logger.warning(
                        f"⚠️  Found {len(in_use_tags)} tags that are CURRENTLY in use (real-time check) - these will be skipped"
                    )
                    ...
                    self.logger.info("✓ Real-time usage check: All tags confirmed as unused")

                # Parallelize deletion operations
                import concurrent.futures

                max_workers = min(config_manager.get_max_workers(), len(unique_tags), 10)  # Cap at 10
                ...
                if mongo_cleanup:
                    ...
                    if object_ids_to_clean:
                        ...
                        try:
                            ...
                            self.logger.info(
                                f"Cleaning up MongoDB records for {len(object_ids_to_clean)} successfully deleted images..."
                            )
                            ...
                            for obj_id_str in object_ids_to_clean:
                                try:
                                    ...
                                    if result.deleted_count > 0:
                                        ...
                                        remaining_revs = environment_revisions_collection.count_documents(
                                        ...
                                        if remaining_revs and remaining_revs > 0:
                                            ...
                                            if referencing_models and referencing_models > 0:
                                                ...
                                                if result.deleted_count > 0:
                                                    ...
                                                    self.logger.warning(
                                                        f"  ✗ Record not found in either collection: {obj_id_str}"
                                                    )
                                except Exception as e:
...
Examples:
  ...
  python delete_unused_environments.py --generate-reports --apply
    ...
    parser.add_argument("--output", help="Output file path (default: reports/unused-environments.json)")

    parser.add_argument(
        "--apply", action="store_true", help="Actually delete unused environment tags (default: dry-run)"
    )

    parser.add_argument("--input", help="Input file containing pre-generated unused tags to delete")
...
def main():
    ...
    output_file = args.output or config_manager.get_unused_environments_report_path()
    ...
    try:
        ...
        logger.info("=" * 60)
        if is_delete_mode:
            logger.info("   🗑️  DELETION MODE: Deleting unused environment tags")
            logger.warning("⚠️  Images WILL be deleted from the registry!")
        else:
            logger.info("   🔍 DRY RUN MODE (default): Finding unused environment tags")
        ...
        if is_delete_mode:
            ...
            if args.apply and args.run_registry_gc:
                ...
                logger.info("Running Docker registry garbage collection after unused environment tag deletion...")
            ...
            if unused_tags:
                ...
                logger.info("  python delete_unused_environments.py --apply")

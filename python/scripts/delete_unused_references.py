...
class UnusedReferencesFinder:
    ...
    def __init__(self, registry_url: str, repository: str):
        ...
        self.collection_patterns = {
            "environment_revisions": ["metadata.dockerImageName.tag", "metadata.dockerImageName.repository"],
            "model_versions": ["metadata.builds.slug.image.tag", "metadata.builds.slug.image.repository"],
        }
        ...
        try:
            ...
            if all_references:
                ...
                for ref in sample_refs:
                    self.logger.debug(
                        f"Sample MongoDB ref - full_image: {ref.full_image}, tag: {ref.tag}, repository: {ref.repository}"
                    )
            ...
            for ref in all_references:
                ...
                if ref.collection == "model_versions":
                    ...
                    image_exists = (
                        ref.full_image in existing_images
                        or ref.tag in existing_images
                        or f"{ref.repository}:{ref.tag}" in existing_images
                    )
                ...
                if image_exists:
                    ...
                    self.logger.debug(
                        f"Image not found - full_image: {ref.full_image}, tag: {ref.tag}, repository: {ref.repository}, collection: {ref.collection}"
                    )
                    unused_references.append(ref)

            self.logger.info(
                f"Found {len(unused_references)} unused references and {len(used_references)} used references"
            )
    ...
    def generate_report(self, unused_references: List[ImageReference], used_references: List[ImageReference]) -> Dict:
        ...
        summary = {
            ...
            "unused_percentage": (
                round((len(unused_references) / (len(unused_references) + len(used_references)) * 100), 2)
                if (unused_references or used_references)
                else 0
            ),
        ...
        try:
            ...
            for value in tags_or_ids:
                ...
                if dry_run:
                    ...
                    if result.deleted_count > 0:
                        self.logger.info(
                            f"Deleted {result.deleted_count} documents for {mode}='{value}' in {collection_name}"
                        )
...
Examples:
  ...
  python delete_unused_references.py --apply --force
    ...
    parser.add_argument("--output", help="Output file path (default: reports/unused-references.json)")
    ...
    parser.add_argument("--input", help="Input file containing pre-generated unused references to delete")
    ...
    try:
        ...
        if is_delete_mode:
            ...
            for collection, stats in report["collection_summary"].items():
                logger.info(
                    f"  {collection}: {stats['unused_references']}/{stats['total_references']} unused ({stats['unused_percentage']}%)"
                )
            ...
            if unused_refs:
                ...
                logger.info(
                    "Review the detailed report to identify which MongoDB records reference non-existent Docker images."
                )
                logger.info("\n" + "=" * 60)
